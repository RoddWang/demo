 ...
 *
 * @author wangjian
 * @email jian.wang6@56qq.com
 * @date 2018/7/30 14:09
 **/
public class ArithmeticExpressionNew {
  ...
  private static String infToSuf(String[] exp) {
    ...
    int l = exp.length;
    for (int i = 0; i < l; i++) {
      if ((stack.empty()) && (exp[i].equals("+") || exp[i].equals("-") || exp[i].equals("*")
          || exp[i].equals("/"))) {
        stack.push(exp[i]);
...
//      System.out.println("后缀:"+newStrs);
  ...
  private static String sufToRes(String sufStr) {
    ...
    for (int i = 0; i < exp.length; i++) {
      if (!exp[i].equals("+") && !exp[i].equals("-") && !exp[i].equals("*") && !exp[i]
          .equals("/")) {
        stack.push(exp[i]);
      } else if (exp[i].equals("+")) {
